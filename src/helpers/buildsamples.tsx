...
export default function buildSamples(props: BuildSampleProps): number[][] {
  ...
  // restrict the time axis to the minimum of the composition time and the 
  // generator's duration
//   const timeLimit: number = Math.min(Tc, generator.stopTime - generator.startTime);
//   const sampleCount: number = SAMPLERATE * timeLimit;
...
const deltaSample: number = SAMPLERATE * deltaT;
    ...
    if (!voices[iVoice].muted) {
      ...
      if (maxCloud != 0) {
        ...
        for (let iTime = 0; iTime < nTimes; iTime++) {
          ...
          for (let iCloud = 0; iCloud < nClouds; iCloud++) {
            ...
            for (
              ...
              iSample < sampleStart + cloud[0].length;
              iSample++
            ) {
              voiceSamples[0][iSample] =
                voiceSamples[0][iSample] != undefined
                  ? voiceSamples[0][iSample] +
                    cloud[0][iSample - sampleStart] * trackGain
                  : cloud[0][iSample - sampleStart] * trackGain;
              voiceSamples[1][iSample] =
                voiceSamples[1][iSample] != undefined
                  ? voiceSamples[1][iSample] +
                    cloud[1][iSample - sampleStart] * trackGain
                  : cloud[0][iSample - sampleStart] * trackGain;
            }
          }
          // if (nClouds != 0)
        ...
        for (let i = 0; i < voiceSamples[0].length; i++) {
          samples[0][i] =
            samples[0][i] != undefined
              ? samples[0][i] + voiceSamples[0][i]
              : voiceSamples[0][i];
          samples[1][i] =
            samples[1][i] != undefined
              ? samples[1][i] + voiceSamples[1][i]
              : voiceSamples[1][i];
        }
      }
    }
  ...
  debug.info(
    `buildSamples: ${voices.length} voices built, sample length = ${samples[0].length}`,
  );
  return samples;
}
