...
export async function loadSoundFont(url:string) {
...
export function applyOptions(
  ctx:AudioContext, source:AudioBufferSourceNode, options) {
...
export function getBufferSourceFromSample(
  ctx:AudioContext, sample:Sample, options = {}) {
...
export const mergeGenerators = (preset: Preset, instrument: Instrument, izone: InstrumentZone, pzone: PresetZone) => {
  Object.fromEntries(
    Object.entries(SFgenerators).map(([index, key]) => [
      key,
      getSFGeneratorValue(parseInt(index), izone, instrument, pzone, preset),
    ]),
  );
};
  ...
  const activeZones = preset.zones
    ...
    .map((pzone) => {
      return pzone.instrument.zones
        ...
        .map((izone) => {
          const mergedGenerators = getSFGeneratorValues(izone, pzone, preset);
...
export const startPresetNote = (
  ctx:AudioContext, preset:Preset, midi:number, time = ctx.currentTime) => {
