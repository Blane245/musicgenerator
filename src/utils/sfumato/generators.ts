...
export const defaultSFGeneratorValues = Object.fromEntries(
  Object.entries(DEFAULT_GENERATOR_VALUES).map(([key, value]) => [SFgenerators[key], value]),
);

// export const getGeneratorValue(generator: string, preset, ) {

export const getSFGeneratorValue = (
  index: number,
...
export const getSFGeneratorValues = (izone: InstrumentZone, pzone: PresetZone, preset: Preset) => {
  return Object.fromEntries(
    Array.from(
      ...
      .map((key) => [SFgenerators[key], getSFGeneratorValue(parseInt(key), izone, pzone.instrument, pzone, preset)]),
  );
};
