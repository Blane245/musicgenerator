...
export default function App() {
  const { setScreenHeight, setScreenWidth, setBodyHeight, setVerticalScrollWidth, setMouseDown } = useCMGContext();
  ...
  // TODO vertical scroll width is not being calculated correctly
  useEffect(() => {
    const handleResize = () => {
      setScreenHeight(window.innerHeight);
      setScreenWidth(window.innerWidth);
      setBodyHeight(window.innerHeight - titleHeight - footerHeight);
      setVerticalScrollWidth(window.innerWidth - document.documentElement.clientWidth);
      }
      handleResize();
