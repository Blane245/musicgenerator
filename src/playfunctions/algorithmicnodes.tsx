...
export function getBufferSourceNodesFromAlgorithmic(
  ...
  gen.initialSequence();
  let { beat, note, speed, duration: noteDuration, attack, volume, pan } = gen.getCurrentValues(time - startTime, gen.measureLength, gen.beatCount );
  ...
  while (time < stopTime) {
    ...
    time += interval;
    ({ beat, note, speed, duration:noteDuration, attack, volume, pan } = gen.getCurrentValues(time - startTime, gen.measureLength, gen.beatCount ));
  }
