...
export function intensityPersist(props: IntensityProps) {
  ...
  for (let i = 0; i < sampleLength; i++) {
    ...
    if (currentDuration >= duration) {
      ...
      debug.info(
        "intensityPersist: apply intensity transition at sample",
        transition,
        i,
      );
      ...
      debug.info(
        "intensityPersist: transition picked",
        transition,
        "new end",
        endIntensity,
      );
...
export function intensityRandom(props: IntensityProps) {
  const { sample, parameters, rN } = props;
...
function getIntensityGainProfile(
  ...
  const start: number | undefined = IntensityProfile.get(
    intensityTransition.start,
  )?.dB;
  const end: number | undefined = IntensityProfile.get(
    intensityTransition.end,
  )?.dB;
