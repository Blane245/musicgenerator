...
export default class Compressor {
    ...
    getXML(fcElem: Element): void {
        try {
            this.threshold = getAttributeValue(fcElem, 'threshold', 'float') as number;
            this.knee = getAttributeValue(fcElem, 'knee', 'float') as number;
            this.ratio = getAttributeValue(fcElem, 'ratio', 'float') as number;
            this.release = getAttributeValue(fcElem, 'release', 'float') as number;
            this.attack = getAttributeValue(fcElem, 'attack', 'float') as number;
        } catch {
     ...
     *
     * @param {HTMLElement} elem - the CMGFile xml element
     */
    appendXML(elem: HTMLElement): void {
        elem.setAttribute('threshold', this.threshold.toString());
        elem.setAttribute('knee', this.knee.toString());
        elem.setAttribute('ratio', this.ratio.toString());
        elem.setAttribute('release', this.release.toString());
        elem.setAttribute('attack', this.attack.toString());
    }
