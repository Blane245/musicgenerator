...
export default class Equalizer {
     ...
     */
    front(): BiquadFilterNode { return this.filters[0] };
    /**
     * return the last audionode of the filter chain
     *
     * @returns {BiquadFilterNode} the last filter in the chain
     */
    back(): BiquadFilterNode { return this.filters[BANDCOUNT-1] };

    copy():Equalizer {
        const nE = new Equalizer();
        nE.frequencies = this.frequencies;
        nE.filters = this.filters;
        nE.gains = this.gains;
        nE.context = this.context;
        return nE;
    }
    ...
    getXML(fcElem: Element): void {
        for (let i = 0; i < BANDCOUNT; i++) {
            try {
                this.gains[i] = getAttributeValue(fcElem, `gain${i}`, 'float') as number
            } catch {
                this.gains[i] = 0;
            }
        }
     ...
     *
     * @param {HTMLElement} elem - the XML element to receive the gain attributes
     */
    appendXML(elem: HTMLElement): void {
        for (let i = 0; i < BANDCOUNT; i++) {
            elem.setAttribute(`gain${i}`, this.gains[i].toString())
        }
    }
