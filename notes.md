...
- the Edit Preferences function is currently disabled as the web client protocol will not allow access the the full path of files for security reasons. Similarly, a Recent Files function is not available. Finding a way around this limitation would be wonderful. 
    -  web browsers do not allow access the local user's file system except though the filedialog interface, and then only the file name is accessible throught the filehandle and not the full path. A python TCL/TK interface exists (https://stackoverflow.com/questions/65622991/absolute-path-to-a-local-file-in-the-user-computer) which might work, but only for unix clients.  
    - what if I had a WPF server running on the client that had access to the file system? I would have to have that server running, but it should be able to access the local machine. 
